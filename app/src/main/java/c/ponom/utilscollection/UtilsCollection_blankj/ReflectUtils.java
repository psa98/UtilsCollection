...
public final class ReflectUtils {
            ...
            public int compare(Constructor<?> o1, Constructor<?> o2) {
                ...
                for (int i = 0; i < len; i++) {
                    if (!types1[i].equals(types2[i])) {
                        if (primitiveWrapper(types1[i]).isAssignableFrom(primitiveWrapper(types2[i]))) {
                            return 1;
                        } else {
                            return -1;
                        }
        ...
        if (declaredTypes.length == actualTypes.length) {
            for (int i = 0; i < actualTypes.length; i++) {
                if (actualTypes[i] == NULL.class
                        || primitiveWrapper(declaredTypes[i]).isAssignableFrom(primitiveWrapper(actualTypes[i]))) {
                    continue;
                }
                return false;
            }
    ...
    private Class<?> primitiveWrapper(final Class<?> type) {
        if (type == null) {
            return null;
